...
import { loadManifestSync, getContestArtifacts } from "@/lib/manifest";
...
async function main() {
  try {
    ...
    console.log(`   ✅ Loaded manifest v${manifest.version} with ${manifest.elections.length} elections`);

    // Test finding contest artifacts
    console.log("2. Resolving contest artifacts...");
    const artifacts = getContestArtifacts(manifest, "portland-20241105-gen", "d2-3seat");
    ...
    for (const check of checks) {
      if (check.path && existsSync(check.path)) {
        ...
        console.log(`   ❌ ${check.name}: ${check.path || 'not generated'} (missing)`);
      }
    }
