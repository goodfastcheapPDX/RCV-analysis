...
import { electionIdFrom, contestIdFrom, type ElectionId, type ContestId } from "../src/contracts/ids";
...
async function main() {
  try {
    ...
    const electionArg = args.find(arg => arg.startsWith('--election='))?.split('=')[1];
    const contestArg = args.find(arg => arg.startsWith('--contest='))?.split('=')[1];
    const srcCsvArg = args.find(arg => arg.startsWith('--src-csv='))?.split('=')[1];

    // Set defaults for District 2
    const srcCsv = srcCsvArg || process.env.SRC_CSV || "tests/golden/micro/cvr_small.csv";
    const electionId = (electionArg || electionIdFrom({
      jurisdiction: 'portland',
      date: '2024-11-05',
      kind: 'gen'
    })) as ElectionId;
    const contestId = (contestArg || contestIdFrom({
      districtId: 'd2',
      seatCount: 3
    })) as ContestId;
    ...
    console.log(`  - Total valid ballots: ${firstChoiceResult.stats.total_valid_ballots}`);
    ...
    console.log(`📂 Artifacts created under: data/dev/${electionId}/${contestId}/`);
