...
export const ElectionIdSchema = z.string().regex(/^[a-z]+-\d{8}-[a-z]+$/, "ElectionId must follow pattern: jurisdiction-YYYYMMDD-type");
export const ContestIdSchema = z.string().regex(/^d\d+-\d+seat$/, "ContestId must follow pattern: dN-Nseat");
export const DistrictIdSchema = z.string().regex(/^d\d+$/, "DistrictId must follow pattern: dN");
