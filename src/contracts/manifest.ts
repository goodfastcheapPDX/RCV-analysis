import { z } from "zod";
import { ElectionIdSchema, ContestIdSchema, DistrictIdSchema } from "./ids";
...
export const ArtifactRef = z.object({
  ...
  sha256: z.string().regex(/^[a-f0-9]{64}$/, "SHA256 hash must be 64 hex characters"),
...
export const StvRules = z.object({
  method: z.enum(['meek', 'droop-surplus-first', 'gregory', 'hare']).default('meek'),
...
export const Election = z.object({
  ...
  date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Date must be in YYYY-MM-DD format"), // "2024-11-05"
...
export const Manifest = z.object({
  ...
  inputs: z.record(z.string(), z.object({
    cvr_files: z.array(z.object({
      path: z.string(),
      sha256: z.string().regex(/^[a-f0-9]{64}$/),
    })),
    rules_hash: z.string().regex(/^[a-f0-9]{64}$/).optional(),
  })),
...
export function findElection(manifest: Manifest, electionId: string): Election | undefined {
  return manifest.elections.find(e => e.election_id === electionId);
}

export function findContest(manifest: Manifest, electionId: string, contestId: string): Contest | undefined {
